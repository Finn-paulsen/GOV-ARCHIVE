...
const handleClick = (e) => {
  ...
  return (
    <div>
      ...
      >
        {node.type === 'folder' ? (isOpen ? '📂 ' : '🗂️ ') : node.type === 'file' && /\.(jpg|png|gif)$/i.test(node.name) ? '🖼️ ' : '📄 '}{node.name}
      </div>
      {node.type === 'folder' && isOpen && node.children && node.children.map(child => (
        <FileNode
          key={child.name}
          node={child}

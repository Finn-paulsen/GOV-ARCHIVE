...
var require_xterm_addon_search = __commonJS({
  "node_modules/xterm-addon-search/lib/xterm-addon-search.js"(exports, module) {
    !(function(e, t) {
      ...
      return (() => {
        ...
        class n extends r.Disposable {
          constructor(e3) {
            var i2;
            super(), this._highlightedLines = /* @__PURE__ */ new Set(), this._highlightDecorations = [], this._selectedDecoration = this.register(new r.MutableDisposable()), this._linesCacheTimeoutId = 0, this._onDidChangeResults = this.register(new t2.EventEmitter()), this.onDidChangeResults = this._onDidChangeResults.event, this._highlightLimit = null !== (i2 = null == e3 ? void 0 : e3.highlightLimit) && void 0 !== i2 ? i2 : 1e3;
          }
          activate(e3) {
          ...
          _highlightAllMatches(e3, t3) {
            ...
            for (; r2 && ((null == s2 ? void 0 : s2.row) !== r2.row || (null == s2 ? void 0 : s2.col) !== r2.col) && !(i2.length >= this._highlightLimit); ) s2 = r2, i2.push(s2), r2 = this._find(e3, s2.col + s2.term.length >= this._terminal.cols ? s2.row + 1 : s2.row, s2.col + s2.term.length >= this._terminal.cols ? 0 : s2.col + 1, t3);
            for (const e4 of i2) {
          ...
          _find(e3, t3, i2, s2) {
            var r2;
            if (!this._terminal || !e3 || 0 === e3.length) return null === (r2 = this._terminal) || void 0 === r2 || r2.clearSelection(), void this.clearDecorations();
            ...
            if (o2 = this._findInLine(e3, n2, s2), !o2) for (let i3 = t3 + 1; i3 < this._terminal.buffer.active.baseY + this._terminal.rows && (n2.startRow = i3, n2.startCol = 0, o2 = this._findInLine(e3, n2, s2), !o2); i3++) ;
            return o2;
          }
          _findNextAndSelect(e3, t3) {
            var i2;
            if (!this._terminal || !e3 || 0 === e3.length) return null === (i2 = this._terminal) || void 0 === i2 || i2.clearSelection(), this.clearDecorations(), false;
            ...
            if (!l) for (let i3 = o2 + 1; i3 < this._terminal.buffer.active.baseY + this._terminal.rows && (n2.startRow = i3, n2.startCol = 0, l = this._findInLine(e3, n2, t3), !l); i3++) ;
            if (!l && 0 !== o2) for (let i3 = 0; i3 < o2 && (n2.startRow = i3, n2.startCol = 0, l = this._findInLine(e3, n2, t3), !l); i3++) ;
          ...
          _findPreviousAndSelect(e3, t3) {
            ...
            if (!h && r2 !== this._terminal.buffer.active.baseY + this._terminal.rows - 1) for (let i3 = this._terminal.buffer.active.baseY + this._terminal.rows - 1; i3 >= r2 && (l.startRow = i3, h = this._findInLine(e3, l, t3, n2), !h); i3--) ;
          ...
          _findInLine(e3, t3, i2 = {}, s2 = false) {
            ...
            if (null == h ? void 0 : h.isWrapped) return s2 ? void (t3.startCol += o2.cols) : (t3.startRow--, t3.startCol += o2.cols, this._findInLine(e3, t3, i2));
            ...
            if (i2.regex) {
              const t4 = RegExp(u, "g");
              let i3;
              if (s2) for (; i3 = t4.exec(f.slice(0, _)); ) g = t4.lastIndex - i3[0].length, e3 = i3[0], t4.lastIndex -= e3.length - 1;
              else i3 = t4.exec(f.slice(_)), i3 && i3[0].length > 0 && (g = _ + (t4.lastIndex - i3[0].length), e3 = i3[0]);
            } else s2 ? _ - u.length >= 0 && (g = f.lastIndexOf(u, _ - u.length)) : g = f.indexOf(u, _);
            if (g >= 0) {
              if (i2.wholeWord && !this._isWholeWord(g, f, e3)) return;
              let t4 = 0;
              for (; t4 < d.length - 1 && g >= d[t4 + 1]; ) t4++;
              let s3 = t4;
              for (; s3 < d.length - 1 && g + e3.length >= d[s3 + 1]; ) s3++;
...
export default require_xterm_addon_search();
